...
console.log('--------------------------')
// 10%, 20%, 30%, 40%, 50% 랜덤출력
const conponDB = ['10%','20%','30%','40%','50%']
const conponBtn = document.querySelector('#coupon')

//버튼 클릭 시 쿠폰 5개 중 1개가 랜덤하게 alert() 출력메세지 실행
conponBtn.addEventListener('click',()=>{
    let randomNum = Math.floor(Math.random() * 5)
    alert(conponDB[randomNum])
})

//화면에 접속했을 때 광고 2개가 랜덤 표시
const adDb = [{
    name : '제주도 여행',
    src :'https://i.pinimg.com/736x/9f/93/f3/9f93f3d675eb124188d95cb056bbb186.jpg',
    link :'https://naver.com'
},{
    name : '일본여행',
    src : 'https://i.pinimg.com/1200x/51/9f/d9/519fd9ea277e1341b29fedffe90962cd.jpg',
...
window.addEventListener('load',()=>{
    ...
    adSection.children[0].src = adDb[adRandom].src
    adSection.children[0].alt = adDb[adRandom].alt
})

//더치페이 계산기
//계산하기 클릭 시 총 금액과 인원 수 에 따라 계산결과 출력하기
//금액이 소수점일 경우 소수점 값에 따라 반올림하기

const totalBtn = document.querySelector('#total_btn')
const priceInput = document.querySelector('#price')
const personInput = document.querySelector('#person')
const resultEm = document.querySelector('.result')
console.log(totalBtn,personInput,priceInput,resultEm)


totalBtn.addEventListener('click',()=>{
    const price = Math.round(priceInput.value / personInput.value)
    resultEm.textContent = price.toLocaleString('ko-kr')
})
...
const min2PriceBtn = document.querySelector('#price_btn')
const mindeliveryBtn = document.querySelector('#delivery_btn')
const resultP1 =document.querySelector('.result1')
const resultP2 =document.querySelector('.result2')
const shopListUl = document.querySelector('#shop_list')
console.log (min2PriceBtn,mindeliveryBtn,resultP1,resultP2,shopListUl)

//최저가 찾기 버튼 클릭 시 최저가 표시
min2PriceBtn.addEventListener('click',()=>{
    //최저가 JS
    const min = shopDB2.map(i=>i.price)
    resultP1.textContent = `최저가${Math.min(...min)}원`;
    ...
    //fot~in, for~of, forEach
    for(let i of sort){
        const li =document.createElement('li')
        //li.textContent=i.shop
        li.innerHTML =`<a href="#">${i.shop}</a>` //태그 추가하고 싶으면 innerHTML사용
        li.innerHTML +=`<span>${i.price}원</span>`
        li.innerHTML +=`<em>배송비 : ${i.price}원</em>`
        console.log(li,i.shop)
        shopListUl.appendChild(li)
        //createelement로 생성한 JS태그는 appendChild함수로 대입한다 (문자 추가이기 때문에 innerHTMl은 사용X)
    }
})

//배송비 무료 버튼 클릭 시 배송비 가격 표시
mindeliveryBtn.addEventListener('click',()=>{
    const min = shopDB2.map(i=>i.delivery)
    resultP2.textContent = `배송비${Math.min(...min)}원`;
})
